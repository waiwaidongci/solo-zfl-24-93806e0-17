...
export class RingsService {
  ...
  // ---------- 激活后轮换密钥（旧密钥立即失效）----------
  // 必须显式携带 expectedVersion（客户端当前看到的 keyVersion）：
  //   缺字段/非整数 → 400 missing_version；版本过期 → 409 version_conflict。
  // 并发重复轮换都拿着同一旧版本号时，互斥串行后只有第一个能赢。
  rotate({ deviceId, loft, keeper, role, expectedVersion }) {
    return this.store.mutate(async state => {
      ...
      const asked = Number(expectedVersion);
      if (expectedVersion === undefined || expectedVersion === null || !Number.isInteger(asked)) {
        throw new DomainError("missing_version", 400, { currentVersion: device.keyVersion });
      }
      if (asked !== device.keyVersion) {
