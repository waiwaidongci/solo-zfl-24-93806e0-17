...
- 设备 HMAC 密钥**只在内存中为明文**；落盘时写 AES-256-GCM 信封 `serverKeyEnc`
  （随机 IV + auth tag，磁盘任何位置都不出现明文密钥）。
- **密文绑定设备标识与用途**：GCM 附加认证数据 `AAD = ring-device-hmac|v2|<deviceId>`。
  把甲设备的密钥信封复制/交换到乙设备记录上，重启解密时 AAD 校验失败，
  服务 `key_envelope_mismatch` fail-fast 拒绝启动，甲的密钥无法为乙的签名背书；
  同样无法用另一台设备的密钥签本机 deviceId 通过校验。
...
- **旧数据安全升级**：v0 明文 `serverKey`、v1 无 AAD 信封在启动加载时被识别，
  当次启动内原子重写为绑定设备的 v2 信封，明文随即从磁盘消失；升级后老设备无需重新
  发放即可继续校验，重放/跨棚防护照常生效。
...
```bash
npm test   # 35 项，显式列出测试文件，Node 20 / 22 均可直接运行
```

覆盖：全生命周期、一次性凭证、同环/同鸽唯一、跨棚激活与校验、验签/时间窗/nonce 重放、
并发激活/轮换/撤销只成功一次、落盘失败原子回滚（含加密存储下轮换/撤销回滚）、重启保留、
密钥 GCM 信封加解密与防篡改、磁盘全文扫描无明文密钥、缺字段/旧版本轮换拒绝、
**跨设备交换密钥信封后重启 fail-fast（领域层 + 真实进程）**、
**v0 明文与 v1 无 AAD 信封启动升级为 v2 后原设备仍可校验**、
错误主密钥 fail-fast、真实 HTTP 进程全链路。
